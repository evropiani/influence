...
- **Play with a friend (P2P multiplayer)**: two humans in one match, plus any number of bots. Browsers connect directly to each other over WebRTC — no server, no accounts. The host shares an invite code, the friend answers with a reply code, and the host runs the match while the friend plays live: expanding, walls, farms, outposts and bombs all work for both players. Works in Classic, Battle Royale, King Of The Hill, Domination and The Zone (teams not yet).
